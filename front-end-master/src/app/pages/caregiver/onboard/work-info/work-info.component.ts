...
export class WorkInfoComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    const date = new Date();
    const iterationNUmber = Number(date.getFullYear()) - 1979;
  ...
  setWorkInfoForm() {
    if (this.profileMode === true) {
      ...
      // this.workInfoForm = new FormGroup({
      //   show_employer_option: new FormControl('', [Validators.required]),
      //   current_employer_hospital_name: new FormControl('', [Validators.required, this.validationService.trimValidator, Validators.maxLength(50)]),
      //   current_employer_work_type: new FormControl('', [Validators.required]),
      //   current_employer_month: new FormControl('', [Validators.required]),
      //   current_employer_year: new FormControl('', [Validators.required]),
      //   current_employer_id: new FormControl('', []),
      //   previous_employer_details: this.formBuilder.array([this.createPreviousEmployerDetails()]),
      //   education_details: this.formBuilder.array([this.createEducationDetails()]),
      // });
